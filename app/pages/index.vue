            ...
            <div class="w-1/2 bg-gray-50 p-6 overflow-y-auto">
                ...
                <div v-else-if="generatedRules" class="space-y-4">
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg p-4">
                        <pre
                            class="whitespace-pre-wrap text-sm text-gray-800 font-mono leading-relaxed">{{ generatedRules }}</pre>
                    </div>
                </div>
...
const handleRestoreFromHistory = () => {
    const restoreData = sessionStorage.getItem('restoreRecord')
    if (restoreData) {
        try {
            const record = JSON.parse(restoreData)
            handleRestoreRecord(record)
            // 清除存储的数据
            sessionStorage.removeItem('restoreRecord')
        } catch (error) {
